...
const HomeSubject: React.FC = () => {
  ...
  const [allSubjects, setAllSubjects] = useState<CmsSubject[]>([]);
  const [selectedSubjectsToAdd, setSelectedSubjectsToAdd] = useState<number[]>([]);
  const [subjectSearchKeyword, setSubjectSearchKeyword] = useState('');

  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getHomeRecommendSubjectListAPI({ 
        subjectName: keyword || undefined, 
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this subject recommendation?')) {
      try {
        await homeRecommendSubjectDeleteByIdsAPI({ ids: String(id) });
  ...
  const handleRecommendStatusChange = async (row: SmsHomeRecommendSubject) => {
    const newStatus = row.recommendStatus === 1 ? 0 : 1;
    try {
      await homeRecommendSubjectUpdateRecommendStatusAPI({ ids: String(row.id), recommendStatus: newStatus });
  ...
  const handleSortChange = async (row: SmsHomeRecommendSubject, newSort: number) => {
    try {
      await homeRecommendSubjectUpdateSortAPI({ id: row.id!, sort: newSort });
  ...
  const handleBatchOperate = async () => {
    ...
    try {
      if (operateType === 'recommend' || operateType === 'unrecommend') {
        ...
        await homeRecommendSubjectUpdateRecommendStatusAPI({ ids: selected.join(','), recommendStatus: status });
      } else if (operateType === 'delete') {
        if (!window.confirm('Are you sure you want to delete selected items?')) return;
        await homeRecommendSubjectDeleteByIdsAPI({ ids: selected.join(',') });
      }
  ...
  const fetchSubjectsToAdd = async () => {
    try {
      const res = await getSubjectListAPI({ keyword: subjectSearchKeyword, pageNum: 1, pageSize: 50 });
      setAllSubjects(res.data.list);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleAddSubmit = async () => {
    if (selectedSubjectsToAdd.length === 0) {
      setSnackbar({ open: true, message: 'Please select at least one subject', severity: 'error' });
      return;
    }
    const data: SmsHomeRecommendSubject[] = selectedSubjectsToAdd.map(id => {
    ...
    try {
      await homeRecommendSubjectCreateAPI(data);
      setSnackbar({ open: true, message: 'Subjects added successfully', severity: 'success' });
      setDialogOpen(false);
      setSelectedSubjectsToAdd([]);
      fetchList();
    } catch {
      setSnackbar({ open: true, message: 'Failed to add subjects', severity: 'error' });
    }
  };
  ...
  return (
    <Box>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} sx={{ textAlign: 'center', py: 6 }}>
                    <Typography sx={{ color: 'text.secondary' }}>No subject recommendations found</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                list.map((row) => (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Select Subjects to Recommend</DialogTitle>
        <DialogContent dividers>
          <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
            <TextField
              ...
              fullWidth
            />
            <Button variant="contained" onClick={fetchSubjectsToAdd}>Search</Button>
          </Box>
          <TableContainer>
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={allSubjects.length > 0 && selectedSubjectsToAdd.length === allSubjects.length}
                      onChange={(e) => setSelectedSubjectsToAdd(e.target.checked ? allSubjects.map(b => b.id!) : [])}
                    />
                  </TableCell>
                  <TableCell>Subject Title</TableCell>
                </TableRow>
