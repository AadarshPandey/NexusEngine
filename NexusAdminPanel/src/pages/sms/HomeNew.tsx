...
const HomeNew: React.FC = () => {
  ...
  const fetchList = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getHomeNewProductListAPI({ 
      ...
      console.error('Failed to fetch new product list:', error);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this recommendation?')) {
      try {
        await homeNewProductDeleteByIdsAPI({ ids: String(id) });
  ...
  const handleRecommendStatusChange = async (row: SmsHomeNewProduct) => {
    const newStatus = row.recommendStatus === 1 ? 0 : 1;
    try {
      await homeNewProductUpdateRecommendStatusAPI({ ids: String(row.id), recommendStatus: newStatus });
  ...
  const handleSortChange = async (row: SmsHomeNewProduct, newSort: number) => {
    try {
      await homeNewProductUpdateSortByIdAPI({ id: row.id!, sort: newSort });
  ...
  const handleBatchOperate = async () => {
    ...
    try {
      if (operateType === 'recommend' || operateType === 'unrecommend') {
        ...
        await homeNewProductUpdateRecommendStatusAPI({ ids: selected.join(','), recommendStatus: status });
      } else if (operateType === 'delete') {
        if (!window.confirm('Are you sure you want to delete selected items?')) return;
        await homeNewProductDeleteByIdsAPI({ ids: selected.join(',') });
      }
  ...
  const handleAddSubmit = async () => {
    ...
    const data: SmsHomeNewProduct[] = selectedProductsToAdd.map(id => {
    ...
    try {
      await homeNewProductCreateAPI(data);
  ...
  return (
    <Box>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} sx={{ textAlign: 'center', py: 6 }}>
                    <Typography sx={{ color: 'text.secondary' }}>No product recommendations found</Typography>
                  </TableCell>
                </TableRow>
              ) : (
                list.map((row) => (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Select Products to Recommend</DialogTitle>
